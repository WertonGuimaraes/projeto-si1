...
public class Usuario implements Serializable {
	...
	public Usuario(String login, String senha, String nome, String email, String endereco) {
		...
		if (endereco == null || Util.isEmpty(endereco))
			throw new LoginInvalidoException("Endereço invalido");
		if (nome == null || Util.isEmpty(nome))
			throw new LoginInvalidoException("Nome inválido");
		if (email == null || Util.isEmpty(email))
	 ...
	 * 
	 * @param carona
	 *            que o usuario quer solicitar vaga
	 * @return id da solicitacao
	 */
	public int solicitaVaga(Carona carona) {
		if (carona.getMotorista().login.equals(this.login))
	...
	/***
	 * Permite ao usuario solicitar vaga em uma carona e m ponto de encontro
	 * 
	 * @param carona
	 *            que o usuario quer solicitar vaga
	...
	public ResponseMeetingPoint getResponsePoint(int idSugestao) {
		for (ResponseMeetingPoint response : this.responsesPontosPendentes) {
			if (response.getIdSugestao() == idSugestao)
				return response;
		}
	...
	public RequestMeetingPoint getRequisicaoPoint(int idSugestao) {
		for (RequestMeetingPoint request : this.requisicoesPontosPendentes) {
			if (request.getIdSugestao() == idSugestao)
				return request;
		}
		return null;
	}
	...
	private HistoricoCarona searchHistoricoByCarona(Carona carona) {
		for (HistoricoCarona historico : this.historicos) {
			if (historico.getCarona().equals(carona))
				return historico;
		}
		return null;
	}
	...
	public int getReviewFaltou() {
		int contador = 0;
		for (HistoricoCarona historico : this.historicos) {
			if (historico.getReview().equals("faltou"))
				contador++;
		}
	...
	public int getReviewNaoFaltou() {
		int contador = 0;
		for (HistoricoCarona historico : this.historicos) {
			if (historico.getReview().equals("não faltou"))
				contador++;
		}
	...
	/**
	 * Retorna o numero de caronas do usuario que não funcionaram
	 * 
	...
	public int getReviewNaoFuncionou() {
		int contador = 0;
		for (HistoricoCarona historico : this.historicos) {
			if (historico.getReview().equals("não funcionou"))
				contador++;
		}
	...
	public int getReviewSeguras() {
		int contador = 0;
		for (HistoricoCarona historico : this.historicos) {
			if (historico.getReview().equals("segura"))
				contador++;
		}
	...
	/**
	 * Método acessador para o numero de caronas que o usuario participou
	 * 
	 * @return numero de caronas que compareceu
	 */
	...
	/**
	 * Remove uma solicitacao de carona feita para esse usuario pelo id da carona solicitada
	 * @param idCaronaSolicitada
	 */
	public void removeRequestById(int idCaronaSolicitada){
		for (int i : this.requests.keySet()) {
			if (this.requests.get(i).getId() == idCaronaSolicitada) {
